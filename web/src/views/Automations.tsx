...
const ACTIONS = ["create_task", "notify", "webhook"];
...
export default function Automations() {
  ...
  const create = async () => {
    if (!name.trim()) return;
    const config = actionType === "notify" ? { message } : actionType === "create_task" ? { title: message } : { url: message };
    ...
    <div className="max-w-2xl mx-auto p-8 space-y-8">
      ...
      >
        ...
        <input
          ...
          placeholder={actionType === "webhook" ? "Webhook URL" : actionType === "create_task" ? "New task title (use {taskTitle})" : "Notification message (use {taskTitle})"}
          className="w-full rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-3 py-2 text-sm"
        />
